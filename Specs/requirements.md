...
- **RF-05 (revisado)** — O InputSystem DEVE suportar, com o MESMO modelo de timing em mobile e desktop:
  ...
  - _(emendado por D-35)_ **A intenção de pulo não é descartada:** o toque que chega com o player ainda no ar é guardado e cobrado no instante do pouso (`JUMP_BUFFER_MS`), e pular vale por `COYOTE_MS` após deixar o chão. Não cria pulo duplo — `startJump()` exige `velocity.y >= 0` e consome a janela. As duas regras vivem no InputSystem/Player, comuns a teclado e toque, então a paridade da RN-08 é preservada.
  - _(emendado por D-35)_ **Exceção do fast-fall durante a recuperação de impacto:** enquanto o jogador se recupera de um escândalo, o tap no ar vale como pedido de PULO, não como descida rápida — sem isso, quem levava o impacto no ar não tinha caminho nenhum para pular até tocar o chão. A janela fecha assim que os pés voltam ao chão; fora dela, o tap no ar continua sendo descida rápida.
...
- **RF-08** — ENQUANTO a partida ocorre, o ScoreSystem DEVE incrementar a pontuação por distância/tempo sobrevivido. _(emendado por D-37: ao cruzar a bandeira de chegada, a ALTURA do player vale bônus — bronze/prata/ouro. Concedido em VOTOS, para a fórmula validada pela Edge Function (RN-04) continuar fechando.)_
